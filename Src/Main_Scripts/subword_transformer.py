...
class SubwordTokenizer:
    """Professional subword tokenizer using BPE (Byte Pair Encoding)."""
    ...
    def __init__(self, vocab: Optional[Dict[str, int]] = None, merges: Optional[List[Tuple[str, str]]] = None):
        if vocab is None:
            ...
            self.vocab = {
                ...
                "<user>": 4,   # User message
                "<bot>": 5,    # Bot message
                "<assistant>": 6,  # Assistant message (for OASST compatibility)
            }
            self.next_id = 7
        else:
            self.vocab = vocab.copy()
            self.next_id = max(vocab.values()) + 1 if vocab else 7
        
        # BPE merges (pair -> merged_token)
        self.merges = merges or []
        self.merge_dict = {pair: merged for pair, merged in self.merges}
        
        # Create reverse mapping
        self.id_to_token = {v: k for k, v in self.vocab.items()}
        
        # Word splitting pattern (preserve alphanumeric and handle punctuation)
        self.word_pattern = re.compile(r"\w+|[^\w\s]")
        
        logger.info(f"SubwordTokenizer initialized with {len(self.vocab)} tokens and {len(self.merges)} merges")
    
    def _get_word_chars(self, word: str) -> List[str]:
        """Split word into characters, adding end-of-word marker."""
        if not word:
            return []
        chars = list(word)
        chars[-1] += "</w>"  # End-of-word marker
        return chars
    
    def _get_pairs(self, chars: List[str]) -> set:
        """Get all adjacent pairs in a list of characters."""
        pairs = set()
        prev_char = chars[0]
        for char in chars[1:]:
    ...
    def _merge_pair(self, chars: List[str], pair: Tuple[str, str], replacement: str) -> List[str]:
        """Merge a specific pair in the character list."""
        new_chars = []
        i = 0
        while i < len(chars):
    ...
    def train_from_text(self, text: str, vocab_size: int = 32000, min_freq: int = 2) -> None:
        ...
        # Initialize character vocabulary with word frequencies
        ...
        logger.info(f"Character vocabulary: {len(char_vocab)} chars")
        ...
        logger.info(f"Learning up to {target_merges:,} BPE merges...")
        
        while merges_learned < target_merges:
            # Count all pairs across all words
            pair_counts = defaultdict(int)
            for word, freq in word_freqs.items():
                if word in word_splits and freq >= min_freq:
            ...
            if merged_token not in self.vocab:
                ...
                if merges_learned % 1000 == 0:
                    logger.info(f"Learned {merges_learned:,} merges, vocab size: {len(self.vocab):,}")
            else:
                # If token already exists, just add the merge rule
                self.merges.append(best_pair)
                self.merge_dict[best_pair] = merged_token
        ...
        logger.info(f"BPE training complete!")
        logger.info(f"Final vocabulary size: {len(self.vocab):,}")
        logger.info(f"Total merges learned: {len(self.merges):,}")
        
        # Show some example merges
        if self.merges:
            logger.info("Sample BPE merges:")
            for i, (pair, merged) in enumerate(zip(self.merges[:10], [self.merge_dict[p] for p in self.merges[:10]])):
                logger.info(f"  {pair[0]} + {pair[1]} -> {merged}")
    ...
    def decode(self, token_ids: List[int]) -> str:
        """Decode subword token IDs back to text."""
        if not token_ids:
            return ""
        ...
        # Reconstruct text by joining subwords and handling end-of-word markers
        text = "".join(tokens)
    ...
    def tokenize(self, text: str) -> List[str]:
        """Tokenize text into subword tokens (for debugging)."""
        if not text:
            return []
    ...
    def load_vocab(self, vocab_path: str, merges_path: str) -> None:
        ...
        logger.info(f"Loaded vocabulary from {vocab_path}: {len(self.vocab)} tokens")
        logger.info(f"Loaded merges from {merges_path}: {len(self.merges)} merges")

# Rest of the transformer architecture remains the same, just updating the class name references
...
class SubwordTransformer(nn.Module):
    """Subword-level transformer model for language generation."""
    ...
    def __init__(self, config):
        ...
        logger.info(f"SubwordTransformer initialized:")
...
WordTransformer = SubwordTransformer
WordTokenizer = SubwordTokenizer